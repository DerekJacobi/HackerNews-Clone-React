...
var StoryTop = React.createClass({displayName: "StoryTop",
  ...
  componentDidMount: function() {
    ...
    $.get(src, function(result) {
      var stories = result;
      if (this.isMounted()) {
        otherSrc ="https://hacker-news.firebaseio.com/v0/item/" + this.state.content[i] + "/.json?print=pretty"
        $.get(otherSrc, function(result) {
          storyList = result;
        })
        this.setState({
  ...
  render: function() {
    ...
    //   src ="https://hacker-news.firebaseio.com/v0/item/" + this.state.content[i] + "/.json?print=pretty"
    //   $.get(src, function(result) {
    //     storyList = result
    //     })
    // }
