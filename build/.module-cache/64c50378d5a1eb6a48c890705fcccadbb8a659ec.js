...
var Story = React.createClass({displayName: "Story",
  ...
  storyCreate: function() {
    var stories = this.state.content.map(function(item){
      console.log(item);
    })
  },
  ...
  render: function() {
    var num = "url"
    var url ="https://hacker-news.firebaseio.com/v0/item/10132935.json?print=pretty"
    var storyNodes = this.state.content.map(function(item) {
        return (
            React.createElement("tr", {key: item}, 
                React.createElement("td", null, 
                    React.createElement("p", {className: "score"}, " Hello this is  \"", num, "\"")
                )
            )
        );
    });
